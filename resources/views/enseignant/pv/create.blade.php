...
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-2xl font-bold">Saisir le procès-verbal</h1>
    </div>
    ...
    <div class="bg-white rounded-xl shadow p-6 max-w-3xl">
        <div class="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div><span class="text-gray-500">Étudiant :</span> <span class="font-medium">{{ $soutenance->etudiant->name ?? 'Non défini' }}</span></div>
                <div><span class="text-gray-500">Titre :</span> <span class="font-medium">{{ $soutenance->titre }}</span></div>
                <div><span class="text-gray-500">Date :</span> <span class="font-medium">{{ \Carbon\Carbon::parse($soutenance->date)->format('d/m/Y H:i') }}</span></div>
                <div><span class="text-gray-500">Filière :</span> <span class="font-medium">{{ $soutenance->filiere }}</span></div>
            </div>
        </div>

        <form action="{{ route('enseignant.pv.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-4">
                ...
                <textarea name="observations" id="observations" rows="4"
                          ...
                          placeholder="Observations, commentaires sur la soutenance...">{{ old('observations') }}</textarea>
                @error('observations')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
